...
FrogClawClient مساعد AI لسطح المكتب متكامل يجمع بين الدردشة متعددة المزودين وعرض المحتوى القوي وتكامل الأدوات وبوابة API مدمجة — يعمل كل شيء محلياً مع حماية قوية للبيانات.
...
- **روابط استيراد المزوّدين** — يمكن لمواقع المزوّدين أو لوحاتهم فتح FrogClawClient عبر رابط `frogclawclient://` وملء الاسم و Base URL ومفتاح API ونوع المزوّد بعد تأكيد المستخدم. راجع [إعداد المزوّدين](./guide/providers).
...
يتضمن FrogClawClient وضع Agent مدمج يمكّن الذكاء الاصطناعي من تنفيذ المهام متعددة الخطوات بشكل مستقل مع تحكم دقيق في الأذونات.
...
يتجاوز FrogClawClient الدردشة النصية البسيطة بكثير مع خط أنابيب عرض غني وتفاعلي.
...
- **أدوات مدمجة** — أدوات MCP مدمجة جاهزة للاستخدام مثل `@frogclaw/fetch`.
- **لوحة تنفيذ الأدوات** — تعرض لوحة مرئية كل طلب استدعاء أداة ونتيجة إرجاعه.

## بوابة API

يتضمن FrogClawClient خادم API محلي مدمجاً يحول تطبيق سطح المكتب إلى بوابة AI قوية.
...
تتيح لك البوابة استخدام FrogClawClient كخلفية AI موحدة لجميع أدواتك. وجّه عملاء CLI وامتدادات IDE أو البرامج النصية المخصصة نحو البوابة المحلية واستفد من تدوير المفاتيح وتتبع الاستخدام والتحكم في الوصول.
:::

## البيانات والأمان

بياناتك لا تغادر جهازك أبداً. FrogClawClient مُصمَّم مع الأمان المحلي أولاً في كل طبقة.

- **تشفير AES-256** — مفاتيح API والبيانات الحساسة الأخرى مشفرة محلياً بـ AES-256.
- **مجلدات بيانات معزولة** — حالة التطبيق في `~/.frogclaw/`. الملفات المرئية للمستخدم في `~/Documents/frogclaw/`.
...
الملف `~/.frogclaw/master.key` هو جذر كل التشفير في FrogClawClient. احتفظ به آمناً وأدرجه في نسخك الاحتياطية.
:::

## تجربة سطح المكتب

FrogClawClient مبني كتطبيق سطح مكتب أصلي مع الصقل والتكامل المتوقعَين من أداة الاستخدام اليومي.
...
- **بدء تلقائي** — تشغيل FrogClawClient اختيارياً عند بدء تشغيل النظام.
- **دعم الوكيل** — تكوين وكلاء HTTP وSOCKS5 للبيئات ذات الوصول المحدود للشبكة.
- **تحديثات تلقائية** — يتحقق FrogClawClient تلقائياً من الإصدارات الجديدة عند الإطلاق.
