...
| Path | Contents |
|------|----------|
| `~/.frogclaw/` | Application state — database, encryption keys, vector DB, SSL certificates |
| `~/Documents/frogclaw/` | User-visible files — images, documents, backups |

::: tip
On Windows the paths are `%USERPROFILE%\.frogclaw\` and `%USERPROFILE%\Documents\frogclaw\`.
:::
...
- **Storage target** — local directory (default: `~/Documents/frogclaw/backups/`) or WebDAV server.
