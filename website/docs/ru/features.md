...
FrogClawClient — полнофункциональный настольный ИИ-ассистент, объединяющий мультипровайдерный чат, мощный рендеринг контента, интеграцию инструментов и встроенный API-шлюз — всё работает локально с надёжной защитой данных.
...
- **Ссылки импорта провайдеров** — Сайты или панели провайдеров могут открывать FrogClawClient через ссылку `frogclawclient://` и после подтверждения пользователя заполнять имя, Base URL, API-ключ и тип провайдера. См. [Настройка провайдеров](./guide/providers).
...
FrogClawClient включает встроенный режим Agent, который позволяет ИИ автономно выполнять многошаговые задачи с детальным контролем разрешений.
...
FrogClawClient выходит далеко за рамки обычного текстового чата с богатым интерактивным конвейером рендеринга.
...
- **Встроенные инструменты** — Готовые к использованию встроенные MCP-инструменты, такие как `@frogclaw/fetch`.
...
FrogClawClient включает встроенный локальный API-сервер, превращающий ваше настольное приложение в мощный ИИ-шлюз.
...
Шлюз позволяет использовать FrogClawClient как единый ИИ-бэкенд для всех ваших инструментов. Настройте CLI-клиенты, расширения IDE или пользовательские скрипты на локальный шлюз и получайте преимущества ротации ключей, отслеживания использования и контроля доступа.
:::

## Данные и безопасность

Ваши данные никогда не покидают вашу машину. FrogClawClient разработан с приоритетом локальной безопасности на каждом уровне.

- **Шифрование AES-256** — Ключи API и другие конфиденциальные данные шифруются локально с помощью AES-256.
- **Изолированные каталоги данных** — Состояние приложения находится в `~/.frogclaw/`. Видимые пользователю файлы хранятся в `~/Documents/frogclaw/`.
...
Файл `~/.frogclaw/master.key` — это корень всего шифрования в FrogClawClient. Храните его в безопасности и включайте в свои резервные копии. Если этот ключ потерян, зашифрованные данные не могут быть восстановлены.
:::

## Опыт рабочего стола

FrogClawClient создан как нативное настольное приложение с полировкой и интеграцией, которые вы ожидаете от инструмента для ежедневного использования.
...
- **Автозапуск** — Опционально запускайте FrogClawClient при старте системы.
- **Поддержка прокси** — Настройте HTTP и SOCKS5 прокси для сред с ограниченным доступом к сети.
- **Автоматические обновления** — FrogClawClient автоматически проверяет наличие новых версий при запуске.
