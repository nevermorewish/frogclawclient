...
FrogClaw 是一款功能全面的 AI 桌面客户端，覆盖了从多模型对话、内容渲染、知识管理到 API 网关等完整的 AI 工作流。以下是各项功能的详细介绍。

## 对话与模型

FrogClaw 提供灵活强大的多模型对话能力，让你在一个客户端中管理所有 AI 服务。

- **多供应商支持** — 兼容 OpenAI、Anthropic Claude、Google Gemini 等主流服务商，以及所有 OpenAI 兼容 API 格式的自定义端点
- **服务商链接导入** — 服务商官网或后台可以通过 `FrogClaw://` 链接拉起 FrogClaw，在用户确认后自动填充名称、Base URL、API Key 和服务商类型。查看[配置服务商](./guide/providers#从网页链接导入服务商)。
...
::: tip 提示
配置多个 API Key 后，FrogClaw 会自动在请求之间轮换密钥，即使单个密钥触发速率限制也不会中断使用。
:::

## AI Agent

FrogClaw 内置 Agent 模式，让 AI 能够自主执行多步骤任务，并提供精细的权限控制。
...
FrogClaw 内置一个功能完整的本地 API 网关，让你的其他工具也能共享 AI 服务配置。
...
::: tip 使用场景
通过 API 网关，你可以让 Cursor、VS Code Copilot、命令行工具等第三方客户端统一经由 FrogClaw 转发请求，实现密钥统一管理和用量集中监控。
:::

## 数据与安全

FrogClaw 将隐私和数据安全放在首位，所有数据完全存储在本地。
