...
FrogClaw 支持接入多种 AI 服务商，你可以同时配置多个服务商并在对话中自由切换。

## 支持的服务商

FrogClaw 内置以下服务商类型，并支持任何兼容 OpenAI API 格式的自定义端点：
...
::: tip 密钥安全
FrogClaw 使用 AES-256 加密存储所有 API 密钥，密钥数据保存在本地 `~/.frogclaw/frogclaw.db` 中，不会上传到任何外部服务器。
:::
...
服务商官网、中转服务后台、私有模型平台或本地网关页面，可以提供一个 **在 FrogClaw 中打开** 的链接。用户点击后，浏览器会拉起 FrogClaw 桌面端，FrogClaw 会进入 **设置 → 服务商**，弹出确认框，并在用户确认后导入服务商配置。

### 用户使用流程

1. 安装并打开支持服务商链接导入的 FrogClaw 版本。
2. 在浏览器中点击服务商页面提供的 **在 FrogClaw 中打开** 链接。
3. 在 FrogClaw 确认框中检查服务商名称、Base URL、服务商类型和 API Key 前缀。
4. 点击确认后，FrogClaw 会按 **Base URL + 类型** 复用已有服务商；如果不存在则新建服务商，并在密钥不存在时追加 API Key。

FrogClaw 不会自动验证 API Key，也不会自动同步模型列表。导入后你可以点击 **获取模型**，或手动添加模型 ID。
...
| `apikey` | 是 | 要保存到 FrogClaw 的 API Key；确认框只展示前缀，不展示完整密钥 |
| `type` | 是 | 服务商类型；可选值：`openai`、`openai_responses`、`anthropic`、`gemini`、`custom` |

`baseurl` 支持 FrogClaw 既有的强制后缀语义，例如 `https://example.com!`。通过链接导入时不会设置 `api_path`，FrogClaw 会继续按所选服务商类型使用默认路径逻辑。
...
```html
<a id="open-frogclaw" href="#">在 FrogClaw 中打开</a>
...
::: tip 测试提示
`FrogClaw://` 是桌面应用注册到系统的自定义协议。仅运行官网或 Vite 开发服务不会注册协议；如果点击后没有拉起 FrogClaw，请先安装或重新构建最新 FrogClaw 桌面端。
:::

## 多密钥轮询

FrogClaw 支持为同一个服务商配置多个 API 密钥，实现自动轮换：
...
当配置了多个密钥时，FrogClaw 会在每次请求时自动轮换使用不同的密钥。
...
点击 **获取模型** 按钮，FrogClaw 会调用服务商的模型列表 API，自动拉取当前可用的全部模型。拉取后的模型会显示在列表中供你选择。
...
FrogClaw 支持连接任何兼容 OpenAI API 格式的服务，以下是常见场景的配置示例。
...
FrogClaw 支持为不同用途设置默认模型，在 **设置 → 默认模型** 中配置：
