...
FrogClaw ist ein voll ausgestatteter Desktop-KI-Assistent, der Multi-Provider-Chat, leistungsstarkes Content-Rendering, Tool-Integration und ein integriertes API-Gateway kombiniert — alles läuft lokal mit starker Datensicherheit.
...
- **Anbieter-Importlinks** — Anbieter-Websites oder Dashboards können FrogClaw über einen `FrogClaw://`-Link öffnen und nach Benutzerbestätigung Name, Base URL, API-Schlüssel und Anbietertyp vorausfüllen. Siehe [Anbieter konfigurieren](./guide/providers).
...
FrogClaw enthält einen integrierten Agent-Modus, der es der KI ermöglicht, mehrstufige Aufgaben autonom mit feingranularer Berechtigungskontrolle auszuführen.
...
FrogClaw geht weit über einfachen Textchat hinaus mit einer reichhaltigen, interaktiven Rendering-Pipeline.
...
FrogClaw enthält einen integrierten lokalen API-Server, der Ihre Desktop-App in ein leistungsstarkes KI-Gateway verwandelt.
...
Das Gateway ermöglicht es Ihnen, FrogClaw als einheitliches KI-Backend für alle Ihre Tools zu verwenden. Konfigurieren Sie Ihre CLI-Clients, IDE-Erweiterungen oder benutzerdefinierten Skripte auf das lokale Gateway.
:::

## Daten und Sicherheit

Ihre Daten verlassen nie Ihren Computer. FrogClaw ist mit lokal-first Sicherheit auf jeder Ebene konzipiert.
...
Die Datei `~/.frogclaw/master.key` ist die Wurzel aller Verschlüsselung in FrogClaw. Bewahren Sie sie sicher auf und fügen Sie sie in Ihre Backups ein.
:::

## Desktop-Erlebnis

FrogClaw ist als native Desktop-Anwendung mit der Politur und Integration aufgebaut, die Sie von einem täglich genutzten Tool erwarten.
...
- **Auto-Start** — Starten Sie FrogClaw optional beim Systemstart.
- **Proxy-Unterstützung** — Konfigurieren Sie HTTP- und SOCKS5-Proxys für Umgebungen mit eingeschränktem Netzwerkzugang.
- **Automatische Updates** — FrogClaw prüft beim Start automatisch auf neue Versionen.
