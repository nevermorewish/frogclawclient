...
FrogClawClient ist ein voll ausgestatteter Desktop-KI-Assistent, der Multi-Provider-Chat, leistungsstarkes Content-Rendering, Tool-Integration und ein integriertes API-Gateway kombiniert — alles läuft lokal mit starker Datensicherheit.
...
- **Anbieter-Importlinks** — Anbieter-Websites oder Dashboards können FrogClawClient über einen `frogclawclient://`-Link öffnen und nach Benutzerbestätigung Name, Base URL, API-Schlüssel und Anbietertyp vorausfüllen. Siehe [Anbieter konfigurieren](./guide/providers).
...
FrogClawClient enthält einen integrierten Agent-Modus, der es der KI ermöglicht, mehrstufige Aufgaben autonom mit feingranularer Berechtigungskontrolle auszuführen.
...
FrogClawClient geht weit über einfachen Textchat hinaus mit einer reichhaltigen, interaktiven Rendering-Pipeline.
...
- **Integrierte Tools** — Sofort einsatzbereite integrierte MCP-Tools wie `@frogclaw/fetch`.
- **Tool-Ausführungspanel** — Ein visuelles Panel zeigt jede Tool-Aufrufanfrage und ihr Rückgabeergebnis an.

## API-Gateway

FrogClawClient enthält einen integrierten lokalen API-Server, der Ihre Desktop-App in ein leistungsstarkes KI-Gateway verwandelt.
...
Das Gateway ermöglicht es Ihnen, FrogClawClient als einheitliches KI-Backend für alle Ihre Tools zu verwenden. Konfigurieren Sie Ihre CLI-Clients, IDE-Erweiterungen oder benutzerdefinierten Skripte auf das lokale Gateway.
:::

## Daten und Sicherheit

Ihre Daten verlassen nie Ihren Computer. FrogClawClient ist mit lokal-first Sicherheit auf jeder Ebene konzipiert.

- **AES-256-Verschlüsselung** — API-Schlüssel und andere sensible Daten werden lokal mit AES-256 verschlüsselt.
- **Isolierte Datenverzeichnisse** — Anwendungszustand in `~/.frogclaw/`. Benutzersichtbare Dateien in `~/Documents/frogclaw/`.
...
Die Datei `~/.frogclaw/master.key` ist die Wurzel aller Verschlüsselung in FrogClawClient. Bewahren Sie sie sicher auf und fügen Sie sie in Ihre Backups ein.
:::

## Desktop-Erlebnis

FrogClawClient ist als native Desktop-Anwendung mit der Politur und Integration aufgebaut, die Sie von einem täglich genutzten Tool erwarten.
...
- **Auto-Start** — Starten Sie FrogClawClient optional beim Systemstart.
- **Proxy-Unterstützung** — Konfigurieren Sie HTTP- und SOCKS5-Proxys für Umgebungen mit eingeschränktem Netzwerkzugang.
- **Automatische Updates** — FrogClawClient prüft beim Start automatisch auf neue Versionen.
