...
FrogClaw 支援接入多種 AI 服務供應商，您可以同時設定多個服務供應商並在對話中自由切換。

## 支援的服務供應商

FrogClaw 內建以下服務供應商類型，並支援任何相容 OpenAI API 格式的自訂端點：
...
::: tip 金鑰安全
FrogClaw 使用 AES-256 加密儲存所有 API 金鑰，金鑰資料儲存在本機 `~/.frogclaw/frogclaw.db` 中，不會上傳到任何外部伺服器。
:::
...
服務供應商官網、中繼服務後台、私有模型平台或本機閘道頁面，可以提供一個 **在 FrogClaw 中開啟** 的連結。使用者點擊後，瀏覽器會喚起 FrogClaw 桌面端，FrogClaw 會進入 **設定 → 服務供應商**，顯示確認視窗，並在使用者確認後匯入服務供應商設定。

### 使用流程

1. 安裝並開啟支援服務供應商連結匯入的 FrogClaw 版本。
2. 在瀏覽器中點擊服務頁面提供的 **在 FrogClaw 中開啟** 連結。
3. 在 FrogClaw 確認視窗中檢查服務供應商名稱、Base URL、服務供應商類型和 API Key 前綴。
4. 確認後，FrogClaw 會按 **Base URL + 類型** 重用既有服務供應商；如果不存在則建立新服務供應商，並在金鑰不存在時追加 API Key。

FrogClaw 不會自動驗證 API Key，也不會自動同步模型清單。匯入後可點擊 **取得模型**，或手動新增模型 ID。
...
| `apikey` | 是 | 要儲存到 FrogClaw 的 API Key；確認視窗只顯示前綴，不顯示完整金鑰 |
| `type` | 是 | 服務供應商類型；可選值：`openai`、`openai_responses`、`anthropic`、`gemini`、`custom` |

`baseurl` 支援 FrogClaw 既有的強制後綴語義，例如 `https://example.com!`。透過連結匯入時不會設定 `api_path`，FrogClaw 會繼續按所選服務供應商類型使用預設路徑邏輯。
...
```html
<a id="open-frogclaw" href="#">在 FrogClaw 中開啟</a>
...
::: tip 測試提示
`FrogClaw://` 是桌面應用註冊到系統的自訂協定。僅執行官網或 Vite 開發服務不會註冊協定；如果點擊後沒有喚起 FrogClaw，請先安裝或重新建置最新 FrogClaw 桌面端。
:::

## 多金鑰輪詢

FrogClaw 支援為同一個服務供應商設定多個 API 金鑰，實現自動輪換：
...
當設定了多個金鑰時，FrogClaw 會在每次請求時自動輪換使用不同的金鑰。
...
點擊 **取得模型** 按鈕，FrogClaw 會呼叫服務供應商的模型清單 API，自動拉取目前可用的全部模型。拉取後的模型會顯示在清單中供您選擇。
...
FrogClaw 支援連接任何相容 OpenAI API 格式的服務，以下是常見場景的設定範例。
...
FrogClaw 支援為不同用途設定預設模型，在 **設定 → 預設模型** 中設定：
