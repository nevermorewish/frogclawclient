...
本文记录 FrogClawClient 的 agent 后端演进方向：保留当前 `open-agent-sdk` 作为通用内嵌 agent 框架，同时接入 Codex 作为专用 coding agent 后端。该架构避免把通用业务 agent 和长时间软件工程 agent 混成一套实现。
...
- `open-agent-sdk`：通用 agent 框架，适合嵌入 FrogClawClient 的业务功能。
...
3. 在 FrogClawClient 内部建立统一的 agent 后端抽象。
...
核心原则：UI 只面对 FrogClawClient 自己定义的 agent API，不直接面对 `open_agent_sdk::Agent` 或 Codex protocol 内部类型。
...
定义 FrogClawClient 自己的后端接口，隐藏具体实现。
...
- 让 Codex 通过 MCP 调用 FrogClawClient 暴露的业务工具。
...
1. FrogClawClient 自己维护统一的 agent session metadata。
2. `OpenAgentBackend` 可以继续使用自身 transcript/session，但需要把外部可见 session id 映射到 FrogClawClient session id。
3. `CodexBackend` 可以维护 Codex thread/rollout/session id 到 FrogClawClient session id 的映射。
4. 不把 Codex 的内部 transcript 路径直接暴露给 UI。
5. 长任务恢复时，以 FrogClawClient session id 为入口，再查具体后端 session id。
...
- 两套后端事件模型不一致。
- Codex 权限和 FrogClawClient 存储策略冲突。
- CLI 子进程方式可能不稳定。
...
- 用 FrogClawClient 自己的 `AgentBackend` 和 `AgentEvent` 隔离实现。
- 第一阶段只做 adapter，不重写现有业务逻辑。
- CodexBackend 先只读，再逐步开放写入和命令执行。
- 权限请求必须统一进入 FrogClawClient 的审批/策略层。
